...
const syntaxHighlight = (code: string, lang: string) => {
  ...
  const applyBase = () => {
    highlightedCode = highlightedCode
      .replace(stringPattern.double, '<span style="color: #ce9178">"$1"</span>')
      .replace(stringPattern.single, "<span style=\"color: #ce9178\">'$1'</span>");
  };

  const applyComments = (commentRegexes: RegExp[]) => {
    for (const regex of commentRegexes) {
      highlightedCode = highlightedCode.replace(regex, '<span style="color: #6a9955">$&</span>');
    }
  };

  if (lang === 'javascript' || lang === 'js') {
    applyComments(commentPatterns.js);
    applyBase();
    highlightedCode = highlightedCode
    ...
    applyComments(commentPatterns.py);
    applyBase();
    highlightedCode = highlightedCode
    ...
    applyComments(commentPatterns.lua);
    applyBase();
    highlightedCode = highlightedCode
    ...
    applyComments(commentPatterns.c);
    applyBase();
    highlightedCode = highlightedCode
    ...
    applyComments(commentPatterns.x3);
    applyBase();
    highlightedCode = highlightedCode
    ...
    applyComments(commentPatterns.visual);
    applyBase();
    highlightedCode = highlightedCode
    ...
    applyComments(commentPatterns.css);
    applyBase();
    highlightedCode = highlightedCode
    ...
    applyComments(commentPatterns.html);
    highlightedCode = highlightedCode
